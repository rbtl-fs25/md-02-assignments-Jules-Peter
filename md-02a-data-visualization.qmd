...
```{r}
###Define data to be used in the plot###
ggplot(gapminder_2007,
       mapping=aes(x=continent,
               y=lifeExp),
       fill=continent)+
  ###Choose plot type and remove outlieres link used:https://www.geeksforgeeks.org/ignore-outliers-in-ggplot2-boxplot-in-r/###
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: geom_bar counts the number of of cases at each x position. In the previous excersice we used the data set "gapminder_summary_2007" were the counts were saved in the column we presented by using geom_col()
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.\
    It shows the number of data points counted on a certain value, or value range, with the colors we are able to see the counts per continent on each value
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:\
    The numbers in breaks() define the values of the x axis, (min,max, value range)

```{r}
ggplot(data=gapminder_2007,
       mapping=aes(x=lifeExp, fill=continent))+
  geom_histogram(col="grey30", breaks=seq(40,85,10))
...
```{r}
ggplot(data=gapminder_2007,
       mapping=aes(x=gdpPercap, y=lifeExp, size=pop ,color=country, ))+
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    A color code for each country that came with the gapminder dataset
